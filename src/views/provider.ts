...
<head>
  ...
  <style>
    *{box-sizing:border-box}body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Noto Sans JP",sans-serif;background:#f3f4f6;color:#111827}
    ...
    h1{font-size:22px;margin:0}.subtitle{color:#6b7280;font-size:13px;margin-top:3px}.actions{display:flex;gap:8px;align-items:center}
    button{font:inherit;cursor:pointer}.logout,.action{border:1px solid #d1d5db;background:#fff;border-radius:7px;padding:9px 14px;font-weight:700}.ready{background:#166534;color:#fff;border-color:#166534}.done{background:#111827;color:#fff;border-color:#111827}.undo{color:#4b5563}
    main{padding:18px;max-width:1400px;margin:auto}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px}.card{background:#fff;border:1px solid #d1d5db;padding:16px}.card.ready-card{border:3px solid #166534}.number{font-size:52px;font-weight:800;line-height:1}.badge{font-size:13px;font-weight:700;color:#6b7280}.ready-card .badge{color:#166534}.items{margin:14px 0;min-height:48px}.item{display:flex;justify-content:space-between;padding:3px 0}.card-actions{display:flex;gap:7px;flex-wrap:wrap}.empty{text-align:center;color:#6b7280;padding:80px 20px;font-size:20px}.toast{position:fixed;left:50%;bottom:24px;transform:translateX(-50%);background:#111827;color:#fff;padding:11px 20px;z-index:5}
  </style>
</head>
<body>
  <header>
    <div><h1>${escapeHtml(locationName)}</h1><div class="subtitle">提供担当画面</div></div>
    <div class="actions"><form method="POST" action="/logout"><button class="logout" type="submit">ログアウト</button></form></div>
  </header>
  ...
  <script nonce="${securityNonce}">
    ...
    function render(tasks) {
      ...
      root.innerHTML = '<div class="grid">' + tasks.map(task => {
        ...
        return '<article class="card ' + (task.status === 'ready' ? 'ready-card' : '') + '"><div style="display:flex;justify-content:space-between;align-items:start"><div class="number">' + pad(task.display_number) + '</div><div class="badge">' + (task.status === 'ready' ? 'お呼び出し中' : '準備中') + '</div></div><div class="items">' + items + '</div><div class="card-actions">' + buttons + '</div></article>';
      }).join('') + '</div>';
    }
...
function renderTasks(tasks: ProviderTask[]): string {
  ...
  return `<div class="grid">${tasks.map(task => `<article class="card ${task.status === "ready" ? "ready-card" : ""}">
    <div style="display:flex;justify-content:space-between;align-items:start"><div class="number">${config.displayNumberPad(task.display_number)}</div><div class="badge">${task.status === "ready" ? "お呼び出し中" : "準備中"}</div></div>
    ...
    <div class="card-actions">${task.status === "preparing" ? `<button class="action ready" data-id="${escapeHtml(task.id)}" data-status="ready">提供可能にする</button>` : `<button class="action done" data-id="${escapeHtml(task.id)}" data-status="handed_over">受渡完了</button><button class="action undo" data-id="${escapeHtml(task.id)}" data-status="preparing">準備中に戻す</button>`}</div>
  </article>`).join("")}</div>`;
}
