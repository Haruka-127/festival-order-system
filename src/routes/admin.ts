...
function renderAdminPage(context: AdminPageContext, activeSection: AdminSection): Response {
    ...
    const items = activeSection === "items" ? getAll<{ id: number; name: string; active: number; sold_out: number; sort_order: number; fulfillment_location_id: number; location_name: string; max_quantity_per_order: number | null; daily_limit: number | null }>(
      db,
    ...
    const orders = activeSection === "orders" ? getAll<{ id: string; display_number: number; status: string; created_at: string; items: string | null; token: string }>(
      db,
    ...
    const users = activeSection === "users" ? getAll<{ id: string; username: string; role: string; staff_type: string; fulfillment_location_id: number | null; location_name: string | null; created_at: string }>(
      db,
    ...
    const locations = ["items", "users", "settings", "locations"].includes(activeSection) ? getAll<{ id: number; name: string; slug: string; active: number; sort_order: number; max_preparing_orders: number | null; max_preparing_units: number | null }>(
      db,
    ...
    const events = activeSection === "history" ? getAll<{ display_number: number; location_name: string | null; event_type: string; from_status: string | null; to_status: string | null; username: string | null; details: string | null; created_at: string }>(
      db,
    ...
    const orderCounts = getOne<{ preparing: number; available: number }>(
      db,
      `SELECT
         COALESCE(SUM(CASE WHEN status = 'preparing' THEN 1 ELSE 0 END), 0) AS preparing,
         COALESCE(SUM(CASE WHEN status = 'available' THEN 1 ELSE 0 END), 0) AS available
       FROM orders`,
    ) ?? { preparing: 0, available: 0 };
    const flashMessages = consumeFlash();

    return new Response(adminPage(items, orders, users, currentNum, securityNonce, locations, settings, events, flashMessages, activeSection, orderCounts), {
...
export const adminRoutes = new Elysia()
  ...
  .post("/api/admin/items", async ({ body, getUser, addFlash }) => {
    ...
    recordAdminAction(db, result, "item_created", { itemId: Number(inserted.lastInsertRowid), name: trimmedName, locationId });
  ...
  .post("/api/admin/items/:id/rename", async ({ params: { id }, body, getUser, addFlash }) => {
    ...
    recordAdminAction(db, result, "item_renamed", { itemId, name: trimmedName });
    return redirectAdmin(addFlash, "success", "商品名を更新しました", "items");
  })

  .post("/api/admin/items/:id/sort", async ({ params: { id }, body, getUser, addFlash }) => {
    const result = requireAdmin(getUser());
    if (result instanceof Response) return result;
    ...
    recordAdminAction(db, result, "item_sort_changed", { itemId, sortOrder: parsedSortOrder });
    return redirectAdmin(addFlash, "success", "表示順を更新しました", "items");
  })

  .post("/api/admin/items/:id/toggle-active", async ({ params: { id }, getUser, addFlash }) => {
    const result = requireAdmin(getUser());
    if (result instanceof Response) return result;
    const itemId = parseItemId(id);
    ...
    if (item) {
      ...
      recordAdminAction(db, result, "item_availability_changed", { itemId, active: !item.active });
    }
    return item
      ? redirectAdmin(addFlash, "success", item.active ? "商品の販売を停止しました" : "商品の販売を再開しました", "items")
      : redirectAdmin(addFlash, "error", "商品が見つかりません", "items");
  })

  .post("/api/admin/items/:id/toggle-soldout", async ({ params: { id }, getUser, addFlash }) => {
    const result = requireAdmin(getUser());
    if (result instanceof Response) return result;
    const itemId = parseItemId(id);
    if (!itemId) return redirectAdmin(addFlash, "error", "不正な商品IDです", "items");
    ...
    if (item) {
      ...
      recordAdminAction(db, result, "item_sold_out_changed", { itemId, soldOut: !item.sold_out });
    }
    wsManager.broadcastToMonitor(getMonitorBoard());
    return item
      ? redirectAdmin(addFlash, "success", item.sold_out ? "売り切れを解除しました" : "商品を売り切れにしました", "items")
      : redirectAdmin(addFlash, "error", "商品が見つかりません", "items");
  })

  .post("/api/admin/items/:id/delete", async ({ params: { id }, getUser, addFlash }) => {
    const result = requireAdmin(getUser());
    if (result instanceof Response) return result;
    const db = getDb();
    const itemId = parseItemId(id);
    ...
    if (usageCount && usageCount.cnt > 0) {
      ...
      recordAdminAction(db, result, "item_disabled", { itemId, reason: "usage_exists" });
      return redirectAdmin(addFlash, "success", "使用実績があるため、販売停止にしました", "items");
    }

    runSql(db, "DELETE FROM items WHERE id = ?", itemId);
    recordAdminAction(db, result, "item_deleted", { itemId });
  ...
  .post("/api/admin/users", async ({ body, getUser, addFlash }) => {
    ...
    recordAdminAction(db, result, "staff_created", { userId: id, username: trimmedUsername, staffType: type, locationId });
  ...
  .post("/api/admin/password", async ({ body, getUser, addFlash, cookie: { session_id } }) => {
    ...
    db.transaction(() => {
      ...
      recordAdminAction(db, result, "admin_password_changed", { userId: result.id });
    })();
  ...
  .post("/api/admin/users/:id/delete", async ({ params: { id }, getUser, addFlash }) => {
    ...
    recordAdminAction(db, result, "staff_deleted", { userId: id, username: target.username });
  ...
  .post("/api/admin/users/:id/settings", ({ params: { id }, body, getUser, addFlash }) => {
    ...
    if (type === "provider" && (!locationId || !getOne(db, "SELECT id FROM fulfillment_locations WHERE id = ? AND active = 1", locationId))) {
      return redirectAdmin(addFlash, "error", "有効な提供場所を指定してください", "users");
    }
    db.transaction(() => {
      runSql(db, "UPDATE users SET staff_type = ?, fulfillment_location_id = ? WHERE id = ?", type, locationId, id);
      runSql(db, "DELETE FROM sessions WHERE user_id = ?", id);
      recordAdminAction(db, result, "staff_settings_changed", { userId: id, staffType: type, locationId });
    })();
  ...
  .post("/api/admin/users/:id/password", async ({ params: { id }, body, getUser, addFlash }) => {
    ...
    db.transaction(() => {
      ...
      runSql(db, "DELETE FROM sessions WHERE user_id = ?", id);
      recordAdminAction(db, result, "staff_password_changed", { userId: id });
    })();
  ...
  .post("/api/admin/reset-numbers", async ({ set, getUser }) => {
    ...
    recordAdminAction(db, result, "display_numbers_reset");
    return { success: true };
  })
  ...
  .post("/api/admin/locations", ({ body, getUser, addFlash }) => {
    ...
    try {
      ...
      recordAdminAction(db, result, "location_created", { locationId: Number(inserted.lastInsertRowid), name: trimmedName, slug: trimmedSlug });
    }
  ...
  .post("/api/admin/locations/:id/settings", ({ params: { id }, body, getUser, addFlash }) => {
    ...
    if (!locationId || !name || name.length > MAX_NAME_LENGTH || sortOrder === null || (maxOrders !== null && (!Number.isInteger(maxOrders) || maxOrders < 1)) || (maxUnits !== null && (!Number.isInteger(maxUnits) || maxUnits < 1))) {
      return redirectAdmin(addFlash, "error", "不正な提供場所設定です", "locations");
    }
    const db = getDb();
    ...
    recordAdminAction(db, result, "location_settings_changed", { locationId, name, sortOrder, maxOrders, maxUnits });
  ...
  .post("/api/admin/locations/:id/toggle-active", ({ params: { id }, getUser, addFlash }) => {
    ...
    recordAdminAction(db, result, "location_availability_changed", { locationId, active: !location.active });
    wsManager.broadcastToMonitor(getMonitorBoard());
  ...
  .post("/api/admin/items/:id/settings", ({ params: { id }, body, getUser, addFlash }) => {
    ...
    if (!getOne(db, "SELECT id FROM fulfillment_locations WHERE id = ? AND active = 1", locationId)) {
      return redirectAdmin(addFlash, "error", "有効な提供場所を指定してください", "items");
    }
    ...
    recordAdminAction(db, result, "item_settings_changed", { itemId, locationId, maxPerOrder, dailyLimit });
  ...
  .post("/api/admin/settings/orders", ({ body, getUser, addFlash }) => {
    ...
    if ((openTime && !timePattern.test(openTime)) || (closeTime && !timePattern.test(closeTime)) || (openTime && closeTime && openTime >= closeTime) || (dailyLimit !== null && (!Number.isInteger(dailyLimit) || dailyLimit < 1)) || !Number.isInteger(maxItems) || maxItems < 1 || maxItems > 100 || !Number.isInteger(maxQuantity) || maxQuantity < 1 || maxQuantity > 10000 || !Number.isInteger(retentionDays) || retentionDays < 1 || retentionDays > 3650) {
      return redirectAdmin(addFlash, "error", "不正な注文設定です", "settings", data.return_to === "advanced" ? "/admin/settings/advanced" : undefined);
    }
    const db = getDb();
    ...
    recordAdminAction(db, result, "order_settings_changed", { enabled: Boolean(enabled), openTime, closeTime, dailyLimit, maxItems, maxQuantity, retentionDays });
  ...
  .post("/api/admin/backup", async ({ getUser, set }) => {
    ...
    try {
      ...
      recordAdminAction(getDb(), result, "backup_created", { filename: backup.filename, bytes: backup.bytes });
