...
店員が注文を登録すると受付番号が発行され、利用客は番号またはQRコードを通じて注文状況を確認できる。商品が提供可能になった際は、モニター画面に番号が表示される。
...
テストは `./data-test/` ディレクトリに一時的なデータベースを作成し、実行後に削除する。
